<template>
    <div class="layer-content clearfix">
        <div class="layer-navbar">
            <div class="layer-navbar_item" :class="{ active: item.activeName===activeName }" v-for="item in navbarJson" @click="selectNavbar(item.activeName)">
    ...
    methods: {
        ...
        confirmPic(url) {
            if(this.isBg===1){
                ...
                //图片裁切
                bus.$emit('navbar-image-handle',true,url);
            }else{
                //添加图片元素
